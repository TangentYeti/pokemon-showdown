...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	honour: {
		onTryBoost(boost, target, source, effect) {
			...
			if (showMsg && !(effect as ActiveMove).secondaries && effect.id !== 'octolock') {
				this.add("-fail", target, "unboost", "[from] ability: Honour", "[of] " + target);
			}
		},
		onModifyMovePriority: -2,
		onModifyMove(move, pokemon) {
	...
	survivalist: {
		onStart(pokemon) {
			if (['strongwinds'].includes(pokemon.effectiveWeather())) {
				const types = ['Bug', 'Flying'];
				if (!pokemon.setType(types)) return;
				this.add('-start', pokemon, 'typechange', types.join('/'), types, '[from] ability: Survivalist');
                this.runEvent('ChangeType', pokemon);
	...
	territorial: {
		onAnySwitchIn(pokemon, source) {
			if (!pokemon.activeTurns && this.effectState.territorial === source.previouslySwitchedIn) return;
			else{
				this.debug('Territorial');
				this.boost({spe: 1})
				this.effectState.territorial = source.previouslySwitchedIn;
			}
		},
		name: "Territorial",
		rating: 3.5,
